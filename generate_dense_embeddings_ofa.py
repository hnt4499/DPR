...
"""
 ...
 Supposed to be used in a 'sharded' way to speed up the process. For One-For-All models only.
"""
import logging
import math
import os
...
from dpr.models import init_ofa_model
...
def main(cfg: DictConfig):
    ...
    saved_state = load_states_from_checkpoint_ofa(cfg.model_file)
    set_cfg_params_from_state(saved_state.encoder_params, cfg)

    logger.info("CFG:")
    logger.info("%s", OmegaConf.to_yaml(cfg))

    tensorizer, ofa_model, _, _, _ = init_ofa_model(
    ...
    prefix_len = len("biencoder.ctx_model.")
    ctx_state = {
        key[prefix_len:]: value
        for (key, value) in saved_state.model_dict.items()
        if key.startswith("biencoder.ctx_model.")
    }
