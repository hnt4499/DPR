"""
A general set of data utilities for preprocessing both biencoder and reader
models.
"""
...
class TokenizedWikipediaPassages(object):
    ...
    def load_data(self):
        if self.data is None:
            ...
            logger.info(
                f"Done. Number of passages: "
                f"{sum(len(shard_data['id']) for shard_data in self.data)}"
            )
...
class GeneralDatasetPreprocessor(torch.utils.data.Dataset):
    """
    General-purpose dataset for both retriever (biencoder) and reader. Input
    data is expected to be output of a trained retriever. This serves as a
    preprocessor as well as a container, to be wrapper by other classes.
    """
    ...
    def __init__(
        ...
        :param files: either path to a single dataset file (*.json) or a glob
            pattern to preprocessed pickle (*.pkl) files.
        :param bm25_retrieval_file: path to the pre-processed BM25 retrieval
            results.
        :param wiki_data: pre-tokenized wikipedia passages.
        :param is_train: whether this dataset is training set or evaluation set.
            Different preprocess settings will be applied to different types of
            training sets.
        :param gold_passages_src: path to the gold passage file.
        :param tensorizer: initialized tensorizer.
        :param run_preprocessing: whether to perform preprocessing. Useful for
            DDP mode, where only one process should do preprocessing while
            other processes wait for its results.
        :param num_workers: number of workers for the preprocessing step.
        :param load_data: whether to load pre-processes data into memory.
            Disable this if you want to pre-process data only
        :param check_pre_tokenized_data: whether to check if the pre-tokenized
            context data is the same as original context passage after
            tokenized.
        :param cfg: configs to overwrite default preprocessing configs. Can be
            partially set (i.e., some key-value pairs may be set and some may
            not.)
        :param compress: whether to compress the outputs and save it as a
            *.json file.
        :param iterator_class: data iterator class. Useful when reading a
            stream of data.
        """
        self.files = files
        ...
        # Iterator class
        allowed_iterator_class = [
            "ShardedDataIterator",
            "ShardedDataStreamIterator",
        ]
        if iterator_class not in allowed_iterator_class:
            raise ValueError(
                f"Invalid iterator class: {iterator_class}. Allowed classes "
                f"are {allowed_iterator_class}"
            )
    ...
    def __iter__(self):
        ...
        preprocessed_data_files = [
            self.preprocessed_data_files[i] for i in indices]
        num_samples_per_file = [self.num_samples_per_file[i] for i in indices]

        # Read data
        for path, num_samples in \
                zip(preprocessed_data_files, num_samples_per_file):
            logger.info(
            ...
            with open(path, "r") as fin:
                for line in fin:
                    if GeneralDatasetPreprocessor._data_post_processing:
    ...
    def load_data(self):
        if self.data is None:
            ...
            # **unprocessed** data files (which may contains regex like
            # ".*.json"). That means specifying processed data files (e.g,
            # "sth.*.pkl") won't work anymore
            preprocessed_data_files = self._get_preprocessed_files([self.files])

            if self.load_data_:
                if self.debugging:
                    logger.info(
                        "Debugging mode is on. Restricting to at most 2 data "
                        "files."
                    )
                ...
                # Compressed
                if self.iterator_class == "ShardedDataIterator":
                    self.data = read_serialized_data_from_files(
                        preprocessed_data_files)
                    ...
                    self.num_samples_per_file = [
                        count_lines_text_file(path)
                        for path in preprocessed_data_files
                    ]
    ...
    def _get_preprocessed_files(
        ...
        # Data may have been serialized and cached before, try to find ones
        # from same dir
        ...
        logger.info(
            "Data are not preprocessed for reader training. Start "
            "pre-processing ..."
        )
        if self.run_preprocessing:
            # Temporarily disable auto-padding to save disk space usage of
            # serialized files
            ...
            # Sync with other processes
            torch.distributed.barrier()
            serialized_files, out_file_prefix = _find_cached_files(
                data_files[0])
        return serialized_files
...
def preprocess_retriever_results(
    ...
    """
    Preprocess the dense retriever outputs (or any compatible file format) into
    the general retriever/reader input data and serializes them into a set of
    files.
    Conversion splits the input data into multiple chunks and processes them in
    parallel. Each chunk results are stored in a separate file with name
    out_file_prefix.{number}.pkl
    :param is_train_set: if the data should be processed for a train set (with
        applicable settings)
    ...
    :param gold_passages_file: optional path for the 'gold passages & questions'
        file. Required to get best results for NQ.
    :param gold_passages_processed_file: path to the preprocessed gold passages
        pickle file. Unlike `gold_passages_file` which contains original gold
        passages, this file should contain processed, matched, 100-word split
        passages that match with the original gold passages.
    :param tensorizer: Tensorizer object for text to model input tensors
        conversions
    :param check_pre_tokenized_data: whether to check if the pre-tokenized
        context data is the same as original context passage after tokenized.
    ...
    :param compress: whether to compress the outputs and save it as a *.json
        file.
    ...
    logger.info(
        f"Loaded {len(samples)} questions + retrieval results from "
        f"{input_file}"
    )
    ...
    if bm25_retrieval_file is not None:
        ...
        logger.info(
        f"Loaded {len(bm25_samples)} BM25 retrieval results from "
        f"{bm25_retrieval_file}"
    )
    else:
    ...
    for file_name in workers.map(_parse_batch, chunks):
        ...
        logger.info(f"Chunks processed {processed}, data saved to {file_name}")
...
def _preprocess_samples_by_chunk(
    ...
    out_file = out_file_prefix + "." + str(chunk_id) + \
        (".json" if compress else ".pkl")
    # Compress
    if compress:
...
PreprocessingCfg = collections.namedtuple(
    ...
    [
        "skip_no_positives",
        # Whether to include the gold passage itself
        "include_gold_passage",
        # Whether the positive passages should only be from the same article as
        # the gold passage
        "gold_page_only_positives",
        # Expand the set of answers; see `answers_processing.py`
        "expand_answers",
        "max_bm25_positives",
        "max_bm25_negatives",
        # Previously every passage has a marker `has_answer` denoting whether it
        # contains the answer string or not (apparently generated by some
        # distant supervision heuristic). If setting
        # `recheck_negatives` to True, passages whose `has_answer` is False are
        # rechecked by newly implemented heuristic to update its `has_answer`
        # attribute. By this, previously-negative passages can become positive
        # (in fact, there is a substantial amount of such passages). It might
        # be the reason the model performance degrades. Hence, this is
        # recommended to be set to False.
        "recheck_negatives",
        # Whether passages containing answer string but not from gold page
        # should be discarded or considered negatives. This option is only
        # applicable when `gold_page_only_positives` is set to True. Note that
        # instead of discarding such passages, it will instead be stored in
        # `distantly_positive_passages`.
        "should_negatives_contain_answer",
        # In retriever (NOT reader) data preparing step, question (but NOT
        # passages) are normalized
        "normalize_questions",
        # Set True for generative reader (FiD), where positive or negative
        # don't matter
        "skip_finding_answer_spans",
        # If answers cannot be found in the passage that has been marked as
        # having answers, do lowercase for both the passage and answers and try
        # to find them again; newly added to support generative readers (e.g.,
        # FiD). This happens when `dense_retrieve.py` uses uncased BERT, which
        # marks a passage as having answer spans although their cases (upper or
        # lower cases) might not match.
...
DEFAULT_PREPROCESSING_CFG_TRAIN = PreprocessingCfg(
    ...
    should_negatives_contain_answer=False,
...
DEFAULT_PREPROCESSING_CFG_TRAIN_GENERATIVE_READER = PreprocessingCfg(
    ...
    include_gold_passage=False,
    gold_page_only_positives=False,
    expand_answers=True,
    max_bm25_positives=10,
    max_bm25_negatives=30,
    recheck_negatives=False,
...
def _preprocess_retriever_data(
    ...
    :param bm25_samples: bm25 retrieval results; list of tuples of tuples of
        (passage_id, score), where passages of each sample are already sorted
        by their scores
    :param gold_info_file: optional path for the 'gold passages & questions'
        file. Required to get best results for NQ
    :param gold_info_processed_file: path to the preprocessed gold passages
        pickle file. Unlike `gold_passages_file` which contains original gold
        passages, this file should contain processed, matched, 100-word split
        passages that match with the original gold passages.
    :param tensorizer: Tensorizer object for text to model input tensors
        conversions
    :param cfg: PreprocessingCfg object with positive and negative passage
        selection parameters
    :param check_pre_tokenized_data: whether to check if the pre-tokenized
        context data is the same as original context passage after tokenized.
    :param is_train_set: if the data should be processed as a train set
    :return: iterable of DataSample objects which can be consumed by the reader
        model
    """
    ...
    for sample, bm25_sample in zip(samples, bm25_samples):
        # Refer to `_get_gold_ctx_dict` for why we need to distinguish between
        # two types of questions. Here `processed_question` refer to tokenized
        # questions, where `question` refer to canonical questions.
        ...
        positive_passages, negative_passages, \
            distantly_positive_passages = passages[1:4]
        bm25_positive_passages, bm25_negative_passages, \
            bm25_distantly_positive_passages = passages[4:7]
    ...
    logger.info(
        f"Number of samples whose gold passage is available: "
        f"{number_samples_with_gold}"
    )
    logger.info(
        f"Number of samples with no positive passages: "
        f"{number_no_positive_samples}"
    )
...
def _select_passages(
    ...
    # Gold context; we want to cover more gold passages, that's why we are
    # matching both `processed_question` and `question` (canonical question).
    if question in processed_gold_passage_map or \
            processed_question in processed_gold_passage_map:
        ...
        gold_ctx = _load_tokens_into_ctx(
            ...
            check_pre_tokenized_data=check_pre_tokenized_data,
        )  # load question, passage title and passage tokens
        gold_ctx = _find_answer_spans(
            ...
            try_lower_case_if_answers_not_found=\
                cfg.try_lower_case_if_answers_not_found,
    ...
    ctxs = [
        _load_tokens_into_ctx(
            ctx, question_token_ids, wiki_data, tensorizer,
            check_pre_tokenized_data=check_pre_tokenized_data,
        )
        for ctx in ctxs
    ]  # load question, passage title and passage tokens
    ...
    ctxs: List[DataPassage] = [
        _find_answer_spans(
            ...
            warn_if_no_answer=ctx.has_answer,
            warn_if_has_answer=(not ctx.has_answer),
    ...
    if is_train_set:
        ...
        if cfg.gold_page_only_positives:
            ...
            selected_bm25_negative_ctxs: List[DataPassage] = \
            ...
            for (
                ...
                for ctx in positives:
                    ...
                    # If it has answer but does not come from gold passage
                    else:
        ...
        # Optionally include gold passage itself if it is still not in the
        # positives list
        if cfg.include_gold_passage:
            if question in gold_passage_map:
                ...
                gold_passage = _find_answer_spans(
                    tensorizer, gold_passage, question,
                    all_answers, answers_token_ids,
                    warn_if_no_answer=False, warn_if_has_answer=False,
                ...
                if not gold_passage.has_answer:
                    logger.warning(
                        f"No answer found in GOLD passage: passage="
                        f"'{gold_passage.passage_text}', question='{question}',"
                        f" answers={answers}, "
                        f"expanded_answers={expanded_answers}",
                    )
        ...
        # positive and negative samples are merged (keeping their original
        # order)
        selected_positive_ctxs = positive_samples
        selected_negative_ctxs = negative_samples
        selected_bm25_positive_ctxs = bm25_positive_samples
    ...
    selected_bm25_positive_ctxs = selected_bm25_positive_ctxs[
        :cfg.max_bm25_positives]
    selected_bm25_negative_ctxs = selected_bm25_negative_ctxs[
        :cfg.max_bm25_negatives]
...
def _load_tokens_into_ctx(
    ...
    if ctx.passage_text is not None:
        ...
        if check_pre_tokenized_data and (
            len(orig_passage_ids) != len(tokens["passage_token_ids"]) or
            not (orig_passage_ids == tokens["passage_token_ids"]).all()
        ):
            raise ValueError(
                f"Passage token mismatch: id: {ctx.id}, orig: "
                f"{orig_passage_ids}, pre-processed: "
                f"{tokens['passage_token_ids']}. If the sequence lengths are "
                f"different, this might be because the maximum length of the "
                f"tokenizer is set differently during pre-processing and "
                f"training."
            )
        ...
        if check_pre_tokenized_data and (
            len(orig_title_ids) != len(tokens["title_token_ids"]) or
            not (orig_title_ids == tokens["title_token_ids"]).all()
        ):
            raise ValueError(
                f"Passage title token mismatch: id: {ctx.id}, orig: "
                f"{orig_title_ids}, pre-processed: {tokens['title_token_ids']}."
                f" If the sequence lengths are different, this might be "
                f"because the maximum length of the tokenizer is set "
                f"differently during pre-processing and training."
            )
...
def _find_answer_spans(
    ...
    # For generative reader (FiD), where positive or negative don't matter
    skip_finding_answer_spans: bool = False,
    ...
    # Lowercase both the passage and answers if the passage originally contains
    # answers
    if len(answers_spans) == 0 and ctx.has_answer and \
            try_lower_case_if_answers_not_found:

        passage_text_lowercased = tensorizer.tensor_to_text(
            tensor=torch.from_numpy(ctx.passage_token_ids),
        ).lower()
        passage_token_ids_lowercased = tensorizer.text_to_tensor(
            text=passage_text_lowercased.lower(),
            add_special_tokens=False)
        ...
        answers_spans = [
            _find_answer_positions(
                passage_token_ids_lowercased,
                answer_token_ids_lowercased,
            )
    ...
    if len(answers_spans) == 0 and (warn_if_no_answer or raise_if_no_answer):
        ...
        message = (
            f"No answer found in passage id={ctx.id} text={passage_text}, "
            f"title={passage_title}, answers={answers}, question={question}"
        )
    ...
    if len(answers_spans) > 0 and (warn_if_has_answer or raise_if_has_answer):
        ...
        passage_title = tensorizer.tensor_to_text(
            torch.from_numpy(ctx.title_token_ids))
        message = (
            f"Answer FOUND in passage id={ctx.id} text={passage_text}, "
            f"title={passage_title}, answers={answers}, question={question}"
        )
...
def _get_gold_ctx_dict(
    file: str,
) -> Tuple[Dict[str, DataPassage], Dict[str, str]]:

    gold_passage_infos = (
        {}
    )  # question|question_tokens -> DataPassage (with title and gold ctx)

    # Original NQ dataset has 2 forms of same question - original, and
    # tokenized. Tokenized form is not fully consisted with the original
    # question if tokenized by some encoder tokenizers
    # Specifically, this is the case for the BERT tokenizer.
    # Depending of which form was used for retriever training and results
    # generation, it may be useful to convert all questions to the canonical
    # original representation.
    original_questions = {}  # tokenized question -> original question (NQ only)
    ...
    for sample in data:
        ...
        question_from_tokens = (
            ...
            if "question_tokens" in sample else question
        )
        ...
        if question in gold_passage_infos:
            ...
            logger.info(
                "Duplicate question gold info: old ctx =%s ",
                rp_exist.passage_text
            )
...
def _get_processed_gold_ctx_dict(file: str) -> Dict[str, DataPassage]:
    ...
    for question, gold_passage_id in data:
        if question in gold_passage_infos:
            logger.info(
                f"Duplicate gold info: question: '{question}', gold passage "
                f"IDs: {gold_passage_infos[question]} and {gold_passage_id}."
            )
            if gold_passage_infos[question] != gold_passage_id:
                logger.warning(
                    f"Each question should have only one unique gold passage; "
                    f"found at least 2: {gold_passage_infos[question]} and "
                    f"{gold_passage_id}. Keeping only the first appearance."
                )
