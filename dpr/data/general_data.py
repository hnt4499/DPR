...
PreprocessingCfg = collections.namedtuple(
    ...
    [
        ...
        # `recheck_negatives` to True, passage whose `has_answer` is False are rechecked by newly implemented
        # heuristic to update its `has_answer` attribute. By this, previously-negative passages can become
        # positive (in fact, there is a substantial amout of such passages). It might be the reason the model
        # performance degrade. Hence, this is recommended to be set to False.
        "recheck_negatives",
    ],
)
